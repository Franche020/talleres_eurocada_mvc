<main class="contenedor">
    <h3>El mejor taller en Torres de la Alameda</h3>

    <p>Torres de la Alameda cuenta con un experto taller de chapa, pintura, mecánica y electricidad para todo tipo de vehículos. Nuestro taller tiene más de 20 años de experiencia y es referente en la zona por su profesionalidad, rapidez y precios competitivos.</p>

    <p>Disponemos de la más moderna maquinaria para realizar cualquier reparación o mantenimiento que necesite su coche, moto, furgoneta o vehículo industrial. Nuestros experimentados chapista, pintores y mecánicos están cualificados para trabajar en cualquier marca y modelo.</p>

    <p>Ofrecemos los siguientes servicios:</p>

    <ul class="flex-vertical">
        <li>Mecánica general: mantenimiento, diagnóstico de averías, reparación del motor, embrague, caja de cambios, frenos, amortiguadores, dirección, etc.</li>
        <li>Chapa y pintura: reparación de abolladuras, pintado, pulido y abrillantado.</li>
        <li>Pre-ITV: revisión previa y reparación para superar la ITV.</li>
        <li>Neumáticos: alineación, equilibrado, cambio y reparación de neumáticos.</li>
        <li>Climatización: carga y reparación de aire acondicionado.</li>
    </ul>

    <p>Disponemos de recambios de primeras marcas y nuestros precios son muy competitivos. Presupuestos sin compromiso.</p>

    <p>Venga a visitarnos o llámenos para concertar cita. Nuestro horario es de lunes a viernes de 9 a 19h.</p>
</main>